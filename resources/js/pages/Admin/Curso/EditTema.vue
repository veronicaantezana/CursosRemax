<template>
  <AdminLayout :title="`Editar Tema - ${tema.nombre}`" subtitle="Edita el tema y sus materiales">
    <div class="space-y-6">
      <div class="flex justify-between items-center">
        <div>
          <h1 class="text-2xl font-bold text-gray-900">Editar Tema</h1>
          <p class="text-gray-600">Modifica el tema: {{ tema.nombre }}</p>
        </div>
        <Link :href="`/admin/cursos/${curso.id}/temas`" class="text-gray-600 hover:text-gray-900 flex items-center gap-2">
        ...
        <template #content>
          <form @submit.prevent="submit" class="space-y-6 p-6">
            ...
            <div class="space-y-4">
              ...
              <div v-if="tema.archivo_path || tema.url" class="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-4">
                <h4 class="font-medium text-gray-900 mb-3">Materiales actuales:</h4>
                
                <!-- Archivo existente -->
                <div v-if="tema.archivo_path" class="flex items-center justify-between p-3 bg-white border border-gray-300 rounded-lg mb-2">
                  <div class="flex items-center gap-3">
                    <i :class="[tema.icono, tema.color, 'text-xl']"></i>
                    <div>
                      <p class="text-sm font-medium text-gray-900">{{ tema.nombre_archivo }}</p>
                      <p class="text-xs text-gray-500 capitalize">{{ tema.tipo_archivo }}</p>
                    </div>
                  </div>
                  <div class="flex gap-1">
                    <Button 
                      @click="descargarArchivo(tema)"
                    ...
                    <Button 
                      @click="eliminarArchivoExistente"
                      severity="danger" 
                      text
                ...
                <div v-if="tema.url" class="flex items-center justify-between p-3 bg-white border border-gray-300 rounded-lg">
                  <div class="flex items-center gap-3">
                    <i class="pi pi-link text-blue-600 text-xl"></i>
                    <div>
                      <p class="text-sm font-medium text-gray-900">URL de video</p>
                      <p class="text-xs text-gray-500 truncate max-w-xs">{{ tema.url }}</p>
                    </div>
                  </div>
                  <div class="flex gap-1">
                    <Button 
                      @click="abrirUrl(tema.url)"
                      class="p-button-text p-button-sm text-blue-600"
                      title="Abrir URL"
                    >
              ...
              <!-- Sección para agregar/reemplazar archivo -->
              <div class="space-y-2">
                <label class="block text-sm font-medium text-gray-700">
                  {{ tema.archivo_path ? 'Reemplazar Archivo' : 'Subir Archivo' }}
                </label>
                <input
                  type="file"
                  ref="archivoInput"
              ...
              <div class="space-y-2">
                <label class="block text-sm font-medium text-gray-700">
                  {{ tema.url ? 'Editar URL' : 'Agregar URL' }}
                </label>
                <InputText
                  v-model="form.url"
                  class="w-full custom-input"
              ...
              <div v-if="!tema.archivo_path && !tema.url && !form.archivo && !form.url" 
                   class="bg-gray-50 border border-gray-200 rounded-lg p-4 text-center">
                <i class="pi pi-info-circle text-gray-400 text-lg mb-2"></i>
                <p class="text-sm text-gray-500">No hay materiales agregados</p>
              </div>
            </div>

            <div class="flex gap-3 pt-4">
              <Link 
                :href="`/admin/cursos/${curso.id}/temas`"
              ...
              >
                ...
                {{ form.processing ? 'Actualizando...' : 'Actualizar Tema' }}
              </Button>
            </div>
          </form>
        </template>
...
const form = useForm({
  nombre: props.tema.nombre,
  duracion: props.tema.duracion,
  archivo: null,
  url: props.tema.url || '',
...
const handleFileUpload = (event) => {
  ...
  if (file) {
    ...
    form.archivo = file
    form.remove_archivo = false
  }
}

const eliminarNuevoArchivo = () => {
  form.archivo = null
...
const eliminarArchivoExistente = () => {
  confirm.require({
    message: '¿Estás seguro de eliminar el archivo actual?',
    header: 'Confirmar eliminación',
    icon: 'pi pi-exclamation-triangle',
    acceptLabel: 'Sí, eliminar',
    rejectLabel: 'Cancelar',
    accept: () => {
      form.remove_archivo = true
      form.archivo = null
    }
  })
}

const eliminarUrlExistente = () => {
  confirm.require({
    message: '¿Estás seguro de eliminar la URL actual?',
    header: 'Confirmar eliminación',
    icon: 'pi pi-exclamation-triangle',
    acceptLabel: 'Sí, eliminar',
    rejectLabel: 'Cancelar',
    accept: () => {
      form.remove_url = true
      form.url = ''
    }
  })
}
...
const submit = () => {
  ...
  if (form.remove_archivo) {
    formData.append('remove_archivo', true)
  }

  if (form.remove_url) {
    formData.append('remove_url', true)
  }

  form.put(`/admin/cursos/${props.curso.id}/temas/${props.tema.id}`, {
    data: formData,
    preserveScroll: true,
    onSuccess: () => {
